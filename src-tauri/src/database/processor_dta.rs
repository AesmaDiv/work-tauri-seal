...
fn parse_type(lines: Vec<String>) -> Option<SType> {
  ...
  if clean_lines.len() == 10 {
    let result = SType {
      ...
      thrlimit  : Some(clean_lines[4].to_string().parse().unwrap_or(0.0)),
      tmplimit  : Some(clean_lines[5].to_string().parse().unwrap_or(0.0)),
      pwrlimit  : Some(clean_lines[6].to_string().parse().unwrap_or(0.0)),
